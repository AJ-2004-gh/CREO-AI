  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await fetch('/api/generate', {
        ...
        body: JSON.stringify({ idea: finalIdea, platform, target_language: targetLanguage, cultural_context: culturalContext }),
      });
        ...
        <div className="flex flex-wrap items-center gap-3 p-4 rounded-2xl" style={{ background: 'rgba(255,255,255,0.7)', border: '1px solid rgba(20,184,166,0.1)', backdropFilter: 'blur(8px)' }}>
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              className={`px-4 py-2 rounded-lg text-sm font-medium transition-all flex items-center gap-2 ${
                mode === 'ondc'
                  ? 'bg-orange-500 text-white shadow-md order-none'
                  : 'bg-white text-gray-700 border border-gray-300 hover:border-gray-400'
              }`}
            >
              🛒 ONDC Seller
